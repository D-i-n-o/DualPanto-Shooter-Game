...
using static CombatC.combatMode;




public class SpeechControlC : MonoBehaviour
{
    ...
    void onSpeechRecognized(string command)
    {
        switch (command) {
            case "Bow": case "Bogen":
                player.GetComponent<CombatC>().SwitchMode(CombatC.combatMode.LONG_RANGE);
                audioSrc[(int)mapToAudio.EQUIP_BOW].Play();
                break;
            case "Sword": case "Schwert":
                audioSrc[(int)mapToAudio.EQUIP_SWORD].Play();
                player.GetComponent<CombatC>().SwitchMode(CombatC.combatMode.CLOSE_RANGE);
                break;
            case "Shild": case "Schild":
                audioSrc[(int)mapToAudio.EQUIP_SHIELD].Play();
                player.GetComponent<CombatC>().SwitchMode(CombatC.combatMode.SHIELD);
                break;
            case "Reload": case "Nachladen":
                audioSrc[(int)mapToAudio.RELOAD].Play();
                player.GetComponent<CombatC>().Reload();
                break;
